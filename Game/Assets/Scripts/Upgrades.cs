...
public class Upgrades {
	public static Upgrade[][] permanent = {
		...
		new Upgrade[] {
			...
			new Upgrade("Advanced armor Mk I", UpgradeTypes.Core, 0, "A more resistent material for the armor that slightly increases the defense.", (Robot r) => {r.defenseBonus += 2; }, (Robot r) => {r.defenseBonus -= 2; }),
		...
		new Upgrade[] {
			...
			new Upgrade("Advanced armor Mk III", UpgradeTypes.Core, 0, "A more resistent material for the armor that considerably increases the defense.", (Robot r) => {r.defenseBonus += 6; }, (Robot r) => {r.defenseBonus -= 6; }),
	...
	public static Upgrade[] temporary = {
		...
		new Upgrade("Repair kit", UpgradeTypes.Consumable, 12, "Recover 50 health.", (Robot r) => { r.UpdateHealth(50); r.CmdPlayClip(r.gameObject, Robot.AudioClips.Repair); }, (Robot r) => {}),
		new Upgrade("Advanced repair kit", UpgradeTypes.Consumable, 20, "Recover 100 health.", (Robot r) => { r.UpdateHealth(100); r.CmdPlayClip(r.gameObject, Robot.AudioClips.Repair); }, (Robot r) => {}),
		new Upgrade("Ultimate repair kit", UpgradeTypes.Consumable, 28, "Recover your full health.", (Robot r) => { r.UpdateHealth(r.healthMax); r.CmdPlayClip(r.gameObject, Robot.AudioClips.Repair); }, (Robot r) => {}),
